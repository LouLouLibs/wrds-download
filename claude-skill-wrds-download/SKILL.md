...
The `wrds-dl` binary must be installed and on the PATH. The user must have WRDS credentials configured via one of:
- Environment variables: `PGUSER` and `PGPASSWORD`
- Saved credentials at `~/.config/wrds-dl/credentials`
- Standard `~/.pgpass` file

If `wrds-dl` is not found, tell the user to install it from https://github.com/LouLouLibs/wrds-download/releases or build from source with `go build`.
